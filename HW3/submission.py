...
class WordSegmentationProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        suc_cost = []
        for a in range(1, len(self.query) - state + 1):
            action = a
            newState = state + action
            cost = self.unigramCost(self.query[state:newState])
            suc_cost.append((action, newState, cost))
...
class VowelInsertionProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        temp_fills = self.possibleFills(self.queryWords[len(state)])
        
        #processing empty value set
        fills = temp_fills if temp_fills else [self.queryWords[len(state)]]
        
        for a in fills:
            action = a
            newState = state + (action,) #add tuple + tuple
            cost = self.bigramCost(state[-1] if state else wordsegUtil.SENTENCE_BEGIN, action)
            suc_cost.append((action, newState, cost))
...
class JointSegmentationInsertionProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        idx, preWord = state
        processing = self.query[idx:]
        combination = [(processing[:i], i) for i in range(1, len(processing) + 1)]
        
        for selected, i in combination:
            for pos_word in self.possibleFills(selected):
                action = pos_word
                newState = (idx + i, pos_word)
                cost = self.bigramCost(preWord, pos_word)
                suc_cost.append((action, newState, cost))
...
class MazeProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        suc_cost += [(direction, nextState, self.moveCost(state, direction)) for direction, nextState in self.possibleMoves(state)]
...
def AStarMazeSearch(start: tuple, goal: tuple, moveCost: Callable[[tuple, str], float],
    ...
    return len(ucs.actions) ## little bit strange,,
